...
def checkpoint_save(state, epoch):
  ...
  torch.save(state, F"{checkpoint_dir_path}/checkpoint_dir/{TraEvN}{40}{startmesh}saved_checkpoint.tar")
def checkpoint_load(loadedcheckpoint):
  print("=> loading checkpoint from", F"{checkpoint_dir_path}/checkpoint_dir/{TraEvN}{40}{startmesh}saved_checkpoint.tar")
...
for BatchSize, LrVal, weight_decay_val in hpmesh[startmesh:endmesh]:
  ...
  if load_model:
    checkpoint_load(torch.load(F"{checkpoint_dir_path}/checkpoint_dir/{TraEvN}{40}{startmesh}saved_checkpoint.tar"))
